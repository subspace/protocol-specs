...
5. `operator_id` ‘s `withdrawals_in_epoch` are incremented with total of `withdraw_shares` from all nominators withdraw requests for the next epoch.
The total of all nominator storage fee deposits (`total_storage_fee_deposit`) is decremented by `storage_fee_refund`.
